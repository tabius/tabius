...
describe('Chords layout lib', () => {
  it('should use chord types from chord-parser-lib', () => {
    ...
    for (const chordName of chordNamesFromLayouts) {
      const layoutChordNameSuffix = chordName.substring(
        chordName.length > 1 && (chordName.charAt(1) === '#' || chordName.charAt(1) === 'b') ? 2 : 1,
      );
  ...
  it('should have 6-fingers layouts with a valid characters', () => {
    const allLayouts = Object.keys(CHORDS_LAYOUTS)
      .map(key => CHORDS_LAYOUTS[key])
      .map(removeFingers);
