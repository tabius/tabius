...
describe('Chords parser, parseChordsLine', () => {
  ...
  it('should correctly parse string tabs', () => {
    // Note: only first line must be parsed.
    expect(parseChordsLine('E--2-3--4-4\nC--3-3-2')).toEqual([]);
    expect(parseChordsLine('B|--2-3--4-4\nC|--3-3-2')).toEqual([]);
    expect(parseChordsLine('G  ||--------1----------1h3p-0--|-----1-------------------||')).toEqual([]);
  });
...
describe('Chords parser, parseChords', () => {
  ...
  it('should recognize braces after chord', () => {
    expect(parseChords('A(III)')).toEqual([{ chord: c('A', 'maj'), startIdx: 0, endIdx: 6 }]);
  ...
  it('should recognize special whitespace chars', () => {
    expect(parseChords('Am/C')).toEqual([{ chord: c('A', 'min', 'C'), startIdx: 0, endIdx: 4 }]);
    expect(parseChords('Am/CC')).toEqual([{ chord: c('A', 'min'), startIdx: 0, endIdx: 2 }]);
  });
});
