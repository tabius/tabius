  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes['songs'] || changes['primarySongCollectionMounts'] || changes['collectionMount']) {
      ...
      if (this.songs.length !== this.primarySongCollections.length) {
        throw new Error(
          `Primary collection mounts count !== songs count: ${this.songs.length}, mounts: ${this.primarySongCollections.length}`,
        );
      }
      for (let i = 0; i < this.songs.length; i++) {
        ...
        if (!collectionMount) {
          console.error(
            `Collection mount is undefined! song: ${song.title}/${song.id}, p: ${primaryCollectionMount}, c: ${collectionMount}`,
          );
          continue;
        }
        this.songItems.push({
          ...
          showPrimaryCollectionLink:
            this.showPrimaryCollectionLinks && !!primaryCollection && collectionMount !== primaryCollectionMount,
